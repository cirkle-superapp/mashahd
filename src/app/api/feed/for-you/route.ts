 ...
 *
 * ROBUSTNESS: the entire main body is wrapped in try/catch. On any error
 * (DB failure, malformed prefs, etc.) we fall back to the trending feed so
 * the home page always renders something instead of crashing.
 */
...
/**
 * Trending fallback used when:
 *   - No `bid` is provided (anonymous users get a non-personalized feed).
 *   - The main personalization pipeline throws (DB error, malformed prefs,
 *     etc.) — we never want the home feed to 500.
 */
async function trendingFallback(limit: number): Promise<NextResponse> {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Any failure in the personalization pipeline → fall back to trending
    // so the home feed always renders something. We intentionally do not
    // log here (the production logger is opt-in) and never expose the
    // error to the client.
    try {
      return await trendingFallback(limit);
    } catch {
